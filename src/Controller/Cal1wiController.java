...
public class Cal1wiController implements Initializable {
    @FXML
    private TableView<Wi1fx> wi1;

    @FXML
    private TableColumn<Wi1fx, Number> id;

    @FXML
    private TableColumn<Wi1fx, String> jour;

    @FXML
    private TableColumn<Wi1fx, String> date;

    @FXML
    private TableColumn<Wi1fx, String> matieres;

    @FXML
    private TableColumn<Wi1fx, String> horaire;

    @FXML
    private TableColumn<Wi1fx, String> duree;

    @FXML
    private TableColumn<Wi1fx, String> salle;
    
    public static EntityManagerFactory emf;
    
    EntityManager em;  
    ObservableList<Wi1fx> data;

    @FXML
    void ajouter(ActionEvent event) throws IOException {
        Stage stage = new Stage();
        FXMLLoader loader = new FXMLLoader();
        loader.setLocation(getClass().getResource("/View/frmcal1wi.fxml"));
        AnchorPane  mainPane = (AnchorPane)loader.load();
        Scene scene = new Scene(mainPane);
        stage.setTitle("Ajouter Nouvelle Examen");
        stage.setScene(scene);
        stage.initStyle(StageStyle.UNDECORATED);
        Frmcal1wiController ctrl = loader.getController();
        stage.showAndWait();
        actualiser();
    }
...
private int entrejour()
    ...
    void imprimer(ActionEvent event) throws DocumentException, SQLException, FileNotFoundException {
        ...
        int k=entrejour();
        int rowno=0;
        rs=pst.executeQuery("SELECT * FROM wi1");
        ...
        PdfWriter.getInstance(d,new FileOutputStream ("1Wi.pdf"));
        PdfPTable pt = new PdfPTable(colnc);
        d.open();
        d.add(new Paragraph("Calendrier Des examen"));
        d.add(new Paragraph("    1Wi  "));
        d.add(new Paragraph("  "));
        d.add(new Paragraph("  "));
    ...
    void modifier(ActionEvent event) throws IOException {
        int row =wi1.getSelectionModel().getSelectedIndex();
        if(row <0)return; 
        Stage stage = new Stage();
        FXMLLoader loader = new FXMLLoader();
        loader.setLocation(getClass().getResource("/View/frmcal1wi.fxml"));
        ...
        stage.initStyle(StageStyle.UNDECORATED);
        Frmcal1wiController ctrl = loader.getController();
        ctrl.setEcom(data.get(row).getIdwi1());
        stage.showAndWait();
        actualiser();
    ...
    void supprimer(ActionEvent event) 
    {
        ...
        q.setParameter("idvic2", data.get(row).getIdwi1());
        Wi1 W= (Wi1) q.getSingleResult();
    ...
    private void actualiser()
    {
        ...
        data.add(new Wi1fx(lst1.getIdwi1(),lst1.getJourWi(),lst1.getDateWi(),lst1.getMatieresWi(),lst1.getHoraireWi(),lst1.getDureeWi(),lst1.getSalleWi()));
        }
        wi1.setItems(data);
        id.setCellValueFactory(cell->cell.getValue().getIdwi1Property());
    ...
    private int   minID()
        { 
            int idmin = 1;
        try {
            pst = connection.prepareStatement("SELECT min(idwi1) FROM wi1");
 ...
 return idmin;
    ...
    private int   maxID()
        { 
            int idm=1;
        try {
            pst = connection.prepareStatement("SELECT max(idwi1) FROM wi1");
            rs = pst.executeQuery();
            while(rs.next())
            {
                idm=rs.getInt(1);
 ...
 return idm;
    ...
    void vider(ActionEvent event) {
        ...
        {
        try{
        String sql ="delete from wi1 where idwi1="+i+"";
