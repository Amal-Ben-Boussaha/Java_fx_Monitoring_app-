...
public class Frmcal2mbdsController implements Initializable {
@FXML
   ...
   private ResultSet rs;
    ...
    public void setEcom(int idmbds2)
     {
        Query q = em.createQuery("SELECT m FROM Mbds2 m WHERE m.idmbds2 = :idmbds2");
        q.setParameter("idmbds2", new Integer(idmbds2));
        mbds2 = (Mbds2) q.getSingleResult();
        jour_txt.setText(mbds2.getJourMbds());
        date_txt.setText(mbds2.getDateMbds());
        matieres_txt.setText(mbds2.getMatieresMbds());
        horaire_txt.setText(mbds2.getHoraireMbds());
        duree_txt.setText(mbds2.getDureeMbds());
        salle_txt.setText(mbds2.getSalleMbds());
        editmode = true;
    ...
    private void enregistrer(ActionEvent event) {
        ...
        {
            mbds2.setJourMbds(jour_txt.getText());
            mbds2.setDateMbds(date_txt.getText());
            mbds2.setMatieresMbds(matieres_txt.getText());
            mbds2.setHoraireMbds(horaire_txt.getText());
            mbds2.setDureeMbds(duree_txt.getText());
            mbds2.setSalleMbds(salle_txt.getText());
...
{ String mStr = matieres_txt.getText().replace( "'" , "''" );
   try{
            String q1 = "SELECT examen FROM mati where classe LIKE '%2MBDS%'  AND examen  LIKE '"+mStr+"%' ";
        ...
        try {
            ...
            int count = 0;
                    ...
                    if(count ==0)
                   {
                     Alert alert = new Alert(Alert.AlertType.ERROR);
                       alert.setHeaderText(null);
                       alert.setContentText("la seance n'existe pas dans la table de séance");
                       alert.show();
                       return false;
...
try{
            String q1 = "SELECT matieres_mbds FROM mbds2 where matieres_mbds like '%"+mStr+"%'";
                   pst = connection.prepareStatement(q1);
...
{   
try{
            String q1 = "SELECT horaire_mbds, date_mbds FROM mbds2 where date_mbds='"+date_txt.getText()+"' and horaire_mbds ='"+horaire_txt.getText()+"'";
                   pst = connection.prepareStatement(q1);
